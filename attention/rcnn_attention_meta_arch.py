"""R-CNN Attention meta-architecture definition.

General tensorflow implementation of R-CNN Attention detection models.

See R-CNN Attention: Ren, Shaoqing, et al.
"R-CNN Attention: Towards real-time object detection with region proposal
...
Implementations of R-CNN Attention models must define a new
RCNNAttentionFeatureExtractor and override three methods: `preprocess`,
...
class RCNNAttentionMetaArch(model.DetectionModel):
  """R-CNN Attention Meta-architecture definition."""
  ...
  def __init__(self,
               ...
               add_gt_boxes_to_rpn=False):
    """RCNNAttentionMetaArch Constructor.
    ...
    Args:
      ...
      feature_extractor: A RCNNAttentionFeatureExtractor object.
      ...
      first_stage_minibatch_size: The "batch size" to use for computing the
        ...
        only called "batch_size" due to terminology from the R-CNN Attention paper.
      ...
      first_stage_nms_score_threshold: Score threshold for non max suppression
        ...
        recommended value for R-CNN Attention is 0.
    ...
    """
    super(RCNNAttentionMetaArch, self).__init__(num_classes=num_classes)
    ...
    self._coloc_target_assigner = coloc_target_assigner.create_coloc_target_assigner(
          'RCNNAttention', 'coloc', unmatched_cls_target=unmatched_cls_target,
          k_shot=k_shot, independent_matching=attention_tree is None)
    ...
    self._attention_tree_only = attention_tree_only
    self._parallel_iterations = parallel_iterations

    self._add_gt_boxes_to_rpn = add_gt_boxes_to_rpn
  ...
  def preprocess(self, inputs):
    ...
    For R-CNN Attention, we perform image resizing in the base class --- each
    class subclassing RCNNAttentionMetaArch is responsible for any additional
    ...
    """
    util.add_extra_tensor('inputs', inputs)
  ...
  def _predict_second_stage(self, rpn_box_encodings,
                            ...
                            anchors,
                            image_shape):
    """Predicts the output tensors from second stage of R-CNN Attention.
    ...
    Returns:
      prediction_dict: a dictionary holding "raw" prediction tensors:
        ...
        3) num_proposals: An int32 tensor of shape [batch_size] representing the
          ...
          which are not since we always pad the number of proposals to be
          `self.max_num_proposals` for each image.
        4) ppoposal_boxes: A float32 tensor of shape
          [batch_size, self.max_num_proposals, 4] representing
          decoded proposal bounding boxes (in absolute coordinates).
        5) mask_predictions: (optional) a 4-D tensor with shape
          ...
          containing instance mask predictions.
    """
    (proposal_boxes_normalized, proposal_scores,
        num_proposals, proposal_matches) = self._postprocess_rpn(
            rpn_box_encodings, rpn_objectness_predictions_with_background,
            anchors, image_shape, return_match_tensor=True)
    ...
    if self._second_stage_convline or self._attention_tree:
      box_classifier_features = tf.reduce_mean(
                         box_classifier_features,
                         [1, 2], keep_dims=True, name='AvgPool')

    if self._second_stage_convline:
      box_classifier_features = self._second_stage_convline.build(
          box_classifier_features[tf.newaxis],
          scope='second_stage_convline')[0]

    if self._attention_tree:
      (box_classifier_features, attention_tree_predictions, attention_tree_cls_predictions,
       proposal_boxes_normalized_reordered, num_proposals) = self._sample_cobjs(
                                                        box_classifier_features,
                                                        proposal_matches,
      ...
      self._tree_debug_tensors = partial(self._attention_tree.debug_tensors,
                                         ...
                                         image_shape)
      proposal_boxes_normalized = proposal_boxes_normalized_reordered

    if self._attention_tree_only:
      combined_shape = shape_utils.combined_static_and_dynamic_shape(
          box_classifier_features)
      refined_box_encodings = tf.zeros(
          [combined_shape[0], self._num_classes, 4],
          dtype=tf.float32)
      class_predictions_with_background = tf.zeros((combined_shape[0],
              self._num_classes+1), dtype=tf.float32)
    else:
  ...
  def _sample_cobjs(self,
                  box_classifier_features,
                  proposal_matches,
                  proposal_boxes):

    proposal_boxes_shape = shape_utils.combined_static_and_dynamic_shape(
                                      proposal_boxes)
    box_shape = shape_utils.combined_static_and_dynamic_shape(
                                      box_classifier_features)
    shape = proposal_boxes_shape[:2] + box_shape[1:]

    # Reshape [batch_size*num_proposals, ...]
    # ==> [batch_size, num_proposals, ...]
    box_classifier_features = tf.reshape(box_classifier_features, shape)
    ...
    (cobj_features, cobj_scores, cobj_cls_scores, _,
     box_classifier_features,
     proposal_boxes) = tree_output.format_output(box_classifier_features,
                                                 proposal_boxes)

    # TODO: this concatenation is not required any more
    # Concatenate the co-object features to box classifier features
    box_classifier_features = tf.concat([
                                        box_classifier_features,
                                        cobj_features
                                        ], axis=4)
    ...
    num_proposals = tf.constant([box_shape[1]
                    for _ in range(box_shape[0])])
    # Reshape [meta_batch_size*k_shot, num_proposals, ...]
    # ==> [meta_batch_size*k_shot*num_proposals, ...]
    box_classifier_features = tf.reshape(box_classifier_features,
        [-1] + box_shape[2:])

    ## Reshape [meta_batch_size*k_shot, num_proposals]
    ## ==> [meta_batch_size*k_shot*num_proposals, 1, ]
    cobj_scores = tf.reshape(cobj_scores, [-1, 1])

    cobj_scores = tf.pad(cobj_scores, [[0,0], [1, 0]])
    return (box_classifier_features, cobj_scores, cobj_cls_scores,
  ...
  def postprocess(self, prediction_dict):
    ...
    with tf.name_scope('SecondStagePostprocessor'):
      ...
      detections_dict.update({
            'rpn_detection_boxes': proposal_boxes,
            'rpn_detection_scores': proposal_scores,
            'rpn_num_detections': num_proposals,
            'rpn_detection_classes': tf.zeros_like(proposal_scores)
      })
  ...
  def _postprocess_rpn(self,
    ...
    if self._is_training and not self._hard_example_miner:
      nms_func = post_processing.batch_multiclass_non_max_suppression
    else:
      nms_func = post_processing_with_resampling.batch_multiclass_non_max_suppression
    (proposal_boxes, proposal_scores, _, _,
     num_proposals) = nms_func(
    ...
    if self._is_training:
      ...
      if not self._hard_example_miner:
        ...
        (proposal_boxes, proposal_scores, proposal_matches,
    ...
    # Check the values num_proposals are equal to self.max_num_proposals
    condition = tf.reduce_all(
        tf.equal(self.max_num_proposals, num_proposals))
    assertion = tf.Assert(condition, [num_proposals, self.max_num_proposals])
    with tf.control_dependencies([assertion]):
      num_proposals = tf.identity(num_proposals)

    # return tensors
    ret = [proposal_boxes, proposal_scores, num_proposals]

    if return_match_tensor:
      ret += [proposal_matches]
    return ret
  ...
  def _unpad_proposals_and_sample_box_classifier_batch(
    ...
    single_image_proposal_match_sample = []
    ...
    for (single_image_proposal_boxes,
      ...
      single_image_proposal_match_sample.append(
          sampled_padded_boxlist.get_field(fields.BoxListFields.classes))

    return (tf.stack(single_image_proposal_box_sample),
            tf.stack(single_image_proposal_score_sample),
            tf.stack(single_image_proposal_match_sample),
  ...
  def _sample_box_classifier_minibatch(self,
    ...
    """
    if self._is_training and self._add_gt_boxes_to_rpn:
      groundtruth_scores = 1.0 - groundtruth_classes_with_background[:,0]
      groundtruth_boxlist.add_field(fields.BoxListFields.scores,
                                              groundtruth_scores)
      proposal_boxlist = box_list_ops.concatenate([proposal_boxlist,
                                                 groundtruth_boxlist])
    ...
    if self._summary_tensors:
      self._debug_summary_collection['Positive_RPN_Proposals'].append(tf.reduce_sum(
          tf.cast(positive_indicator, dtype=tf.int32)))
      self._debug_summary_collection['RPN_Proposals'].append(tf.shape(
                                                          positive_indicator)[0])
    sampled_indices = util.balanced_subsample(
          tf.cast(cls_weights, tf.bool),
          positive_indicator,
          self._second_stage_batch_size,
          self._second_stage_balance_fraction)

    proposal_boxlist.add_field(fields.BoxListFields.classes,
                                    cls_targets)
  ...
  def _postprocess_box_classifier(self,
    ...
    Args:
      box_classifier_features: a 4-D tensor of features
    ...
    box_classifier_features_shape = (
        shape_utils.combined_static_and_dynamic_shape(box_classifier_features))
    box_classifier_features_batch = tf.reshape(
        box_classifier_features,
        [-1, self.max_num_proposals]+box_classifier_features_shape[1:])
    ...
    # tile since it should be multiclass 
    box_classifier_features_batch = tf.tile(box_classifier_features_batch,
    ...
    (nmsed_boxes, nmsed_scores, nmsed_classes, nmsed_masks,
     num_detections) = self._second_stage_nms_fn(
         ...
         masks=box_classifier_features_batch)
    #nmsed_boxes = refined_decoded_boxes_batch[..., 0, :]/512.0
    #nmsed_scores = tf.reduce_max(class_predictions_batch, axis=-1)
    #nmsed_classes = tf.argmax(class_predictions_batch, axis=-1)
    #num_detections = [400]
    #nmsed_masks = None
    ...
    if nmsed_masks is not None:
      detections['detection_feas'] = nmsed_masks[:,:, tf.newaxis]
    if mask_predictions is not None:
  ...
  def loss(self, prediction_dict, scope=None):
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      ...
      loss_dict = self._loss_rpn(
          ...
          groundtruth_boxlists,
          groundtruth_classes_with_background_list)
      if not self._first_stage_only and not self._attention_tree_only:
        loss_dict.update(
            self._loss_box_classifier(
                ...
                groundtruth_classes_with_background_list))
      if self._attention_tree:
        for key, value in self._attention_tree.loss().items():
          name = 'attention_tree_loss_%d'%key
          loss_dict[name] = tf.identity(value, name=name)


      if self._summary_tensors:
        for key, value in self._debug_summary_collection.items():
          if key == 'top_attention_boxes':
            preprocessed_inputs = self._debug_summary_collection['top_attention_images']
            channel_means = [123.68, 116.779, 103.939]
            images = preprocessed_inputs + [[channel_means]]
            top_box_images = tf.image.draw_bounding_boxes(images, value)
            tf.summary.image('Attention_Boxes', top_box_images, max_outputs=self._k_shot)
          elif key == 'top_attention_images':
            pass
          else:
            value = [tf.cast(v, dtype=tf.float32) for v in value]
  ...
  def _loss_box_classifier(self,
    ...
    with tf.name_scope('BoxClassifierLoss'):
      ...
      (batch_cls_targets_with_background, batch_cls_weights, batch_reg_targets,
       batch_reg_weights, _) = coloc_target_assigner.batch_assign_targets(
           self._coloc_target_assigner, proposal_boxlists,
           groundtruth_boxlists, groundtruth_classes_with_background_list)
      util.add_extra_tensor('batch_cls_targets_with_background', batch_cls_targets_with_background)
      util.add_extra_tensor('batch_cls_weights', batch_cls_weights)
      util.add_extra_tensor('batch_reg_targets', batch_reg_targets)
      util.add_extra_tensor('batch_reg_weights', batch_reg_weights)
