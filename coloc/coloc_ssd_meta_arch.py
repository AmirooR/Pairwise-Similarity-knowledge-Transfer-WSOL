"""CoLocSSD Meta-architecture definition.
...
import os_box_predictor as bpredictor
from object_detection.core import model
from object_detection.core import standard_fields as fields
from object_detection.core import target_assigner
from object_detection.utils import shape_utils
from object_detection.utils import static_shape
from util import dict_union, collect_debug
from object_detection.core import box_list_ops
slim = tf.contrib.slim

class CoLocSSDFeatureExtractor(object):
  ...
  def __init__(self,
    ...
    self._endpoints_indicator = endpoints_indicator
    assert(not endpoints_indicator or all(
      [x == '0' or x == '1' for x in endpoints_indicator]))
...
class CoLocSSDMetaArch(model.DetectionModel):
  """CoLocSSD Meta-architecture definition."""

  def __init__(self,
               is_training,
               k_shot,
               dynamic_parameter_learner,
               anchor_generator,
               box_predictor,
               box_coder,
               feature_extractor,
    ...
    Args:
      ...
      k_shot: number images for co-localization of an object
      dynamic_parameter_learner: dynamic convolution learner
      anchor_generator: an anchor_generator.AnchorGenerator object.
      box_predictor: a os_box_predictor.OSBoxPredictor object.
      box_coder: a box_coder.BoxCoder object.
      feature_extractor: a CoLocSSDFeatureExtractor object.
    ...
    """
    super(CoLocSSDMetaArch, self).__init__(num_classes=box_predictor.num_classes)
    self._is_training = is_training
    self.k_shot = k_shot
    self._dynamic_parameter_learner = dynamic_parameter_learner
    ...
    box_predictor.init_variables(
        num_predictions_per_location_list=num_anchors_per_location_list,
        box_code_size=box_coder.code_size,
        dynamic_parameters_scope='InitOSBoxPredictor')
  ...
  def preprocess(self, inputs):
    ...
    Returns:
      preprocessed_inputs: a [batch, height, width, channels] float
  ...
  def predict(self, preprocessed_inputs):
    ...
    with tf.variable_scope(None, self._extract_features_scope,
      ...
      feature_maps = self._feature_extractor.extract_features(
          preprocessed_inputs)
    dynamic_parameters_map = self._predict_dynamic_parameters(feature_maps)
    feature_map_spatial_dims = self._get_feature_map_spatial_dims(feature_maps)
    self._anchors = self._anchor_generator.generate(feature_map_spatial_dims)
    (box_encodings, class_predictions_with_background
    ) = self._add_box_predictions_to_feature_maps(feature_maps, dynamic_parameters_map)
    predictions_dict = {
        'box_encodings': box_encodings,
        'class_predictions_with_background': class_predictions_with_background,
        'feature_maps': feature_maps
    }
    collect_debug('predictions_dict', predictions_dict)
    return predictions_dict

  def _add_box_predictions_to_feature_maps(self, feature_maps, dynamic_parameters_map):
    ...
    # Box predictors receive 5 dimensional features: [meta_batch_size, batch_size, h, w, c]
    # Reshape feature_map [batch_size*k_shot, h, w, c] ==> [batch_size, k_shot, h, w, c]
    in_ = []
    for feature_map in feature_maps:
      in_.append(tf.reshape(feature_map,
                  tf.concat([[-1, self.k_shot], tf.shape(feature_map)[1:]], 0)))

    prediction_dict = self._box_predictor.build(
            in_,
            dynamic_parameters_map,
            scope=None)

    box_encodings = tf.squeeze(
                tf.concat(prediction_dict[bpredictor.BOX_ENCODINGS], axis=1), axis=2)
    class_predictions_with_background = tf.concat(
                prediction_dict[bpredictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
  ...
  def _get_feature_map_channel_dims(self, feature_maps):
    feature_map_shapes = [
        shape_utils.combined_static_and_dynamic_shape(
            feature_map) for feature_map in feature_maps
    ]
    return [shape[3] for shape in feature_map_shapes]


  def _predict_dynamic_parameters(self, features_maps):
    input_depth_list = self._get_feature_map_channel_dims(features_maps)
    shape_map_list = self._box_predictor.dynamic_parameters_shape_map_list(
                                      input_depth_list=input_depth_list)
    dynamic_parameters_map = dict()
    for i, (features_map, shape_map) in enumerate(zip(
      features_maps, shape_map_list)):
      features_map = tf.reshape(features_map,
          tf.concat([[-1, self.k_shot], tf.shape(features_map)[1:]], 0))
      collect_debug('dynamic_parameters/input_{}'.format(i), features_map)
      partial_dynamic_parameters_map = self._dynamic_parameter_learner.learn(
                          features_map, shape_map,
                          scope=self.parameter_learner_scope+'_{}'.format(i))
      collect_debug('dynamic_parameters/output_{}'.format(i), dict(partial_dynamic_parameters_map))
      dict_union(dynamic_parameters_map,
                 partial_dynamic_parameters_map)
    if self._add_summaries:
      for name, tensor in dynamic_parameters_map.items():
        tf.summary.histogram(name, tensor)
    return dynamic_parameters_map

  @property
  def parameter_learner_scope(self):
    return 'ParameterLearner'
  ...
  def postprocess(self, prediction_dict):
    ...
    with tf.name_scope('Postprocessor'):
      ...
      collect_debug('detection_scores', detection_scores)
      ...
      collect_debug('detections', detections)
      return detections
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      (batch_cls_targets, batch_cls_weights, batch_reg_targets,
       batch_reg_weights, match_list) = self._assign_targets(
           ...
           self.groundtruth_lists(fields.BoxListFields.classes)) #FIXME
      if self._add_summaries:
