...
"""Tests for coloc_ssd.coloc.coloc_ssd_meta_arch."""
import functools
import numpy as np
...
class FakeSSDFeatureExtractor(coloc_ssd_meta_arch.CoLocSSDFeatureExtractor):
...
class SsdMetaArchTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    dynamic_parameter_learner = ? #TODO
    ...
    self._num_anchors = 4
    self._code_size = 4
    self._model = coloc_ssd_meta_arch.CoLocSSDMetaArch(
        is_training, self._k_shot, dynamic_parameter_learner, mock_anchor_generator,
        mock_box_predictor, mock_box_coder, fake_feature_extractor, mock_matcher,
        region_similarity_calculator, non_max_suppression_fn, tf.identity,
