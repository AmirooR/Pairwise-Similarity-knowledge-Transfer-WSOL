...
def _get_inputs(input_queue, num_classes, k_shot):
  ...
  """
  assert(num_classes > 0), 'num_classes should be more than 0'
  read_data_list = input_queue.dequeue()
  label_id_offset = 1
  def extract_images_and_targets(read_data):
    image = read_data[fields.CoLocInputDataFields.supportset]
    n = fields.CoLocInputDataFields.groundtruth_boxes + '_{}'
    m = fields.CoLocInputDataFields.groundtruth_classes + '_{}'
    boxes_list, classes_list = [], []
    for k in range(k_shot):
      classes_gt = read_data[m.format(k)]
      classes_gt -= label_id_offset
      assert_class = tf.Assert(tf.reduce_all(tf.less(classes_gt, num_classes)),
                                      [classes_gt, num_classes])

      with tf.control_dependencies([assert_class]):
        classes_gt = util_ops.padded_one_hot_encoding(indices=classes_gt,
                                          depth=num_classes, left_pad=0)
      classes_list.append(classes_gt)
      boxes_list.append(read_data[n.format(k)])
    return image, boxes_list, classes_list
...
def _create_losses(input_queue, create_model_fn):
  ...
  k_shot = detection_model._k_shot
  (images, boxes_list_list, classes_list_list
  ) = _get_inputs(input_queue, detection_model.num_classes, k_shot)

  images_c = [detection_model.preprocess(k)
              for image in images
                for k in tf.split(image,k_shot)]
  images = tf.concat(images_c, 0)
  groundtruth_boxes_list = [k
                    for boxes_list in boxes_list_list
                      for k in boxes_list]
  groundtruth_classes_list = [k for classes_list in classes_list_list
...
def train(create_tensor_dict_fn, create_model_fn, train_config, master, task,
  ...
  with tf.Graph().as_default():
    ...
    if train_config.fine_tune_checkpoint:
      ...
      # Only restore the variables that are in the 
      # checkpoint and have the same size
      available_var_map = {}
      for name, var in var_map.items():
        name2 = name
        #if name2 == 'Conv/weights':
        #  name2 = 'RPNPredictorConv1_114_512/weights'
        #if name2 == 'Conv/biases':
        #  name2 = 'RPNPredictorConv1_114_512/biases'

        if name2 in checkpoint_var_shape:
          if checkpoint_var_shape[name2] == var.shape.as_list():
            available_var_map[name2] = var
          else:
            print('\t * WARNING: invalid shape for var: {}'.format(name2))
        else:
          print('\t * WARNING: invalid variable name: {}'.format(name2))
    ...
    slim.learning.train(
        train_tensor,
        #train_step_fn=learning.train_step, ###
        logdir=train_dir,
        master=master,
        is_chief=is_chief,
