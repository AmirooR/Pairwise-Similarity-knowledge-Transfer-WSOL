...
from rcnn_attention.wrn import evaluator
from rcnn_attention.wrn import mrf_evaluator as mrf_evaluator
...
flags.DEFINE_string('energy_save_name', 'energy', 'save name for energy values when in energy mode')
...
flags.DEFINE_float('unary_scale', 0.0, 'scale of unary potentials')
flags.DEFINE_string('accumulator_save_path', None, "Save directory for graph accumulator")
...
def main(unused_argv):
  ...
  FLAGS.zero_pairwise = False
  if FLAGS.add_mrf:
    if input_k_shot == 1:
      input_k_shot = 2
      FLAGS.zero_pairwise=True
  ...
  mode = None
  aggregate = (eval_config.HasField('aggregation_params') and
               eval_config.aggregation_params.HasField('aggregate') and
               eval_config.aggregation_params.aggregate)
  if aggregate:
    mode = 'aggregate'
    if (eval_config.aggregation_params.HasField('overlap') and
        eval_config.aggregation_params.overlap):
      mode = 'aggregate,overlap'
  ...
  categories = None
  if FLAGS.add_mrf:
    mrf_evaluator.evaluate(create_input_dict_fn_list, input_config_names,
